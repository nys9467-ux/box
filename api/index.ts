  ...
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({ 
        error: "서버에 KOBIS API Key가 설정되지 않았습니다. Vercel 환경 변수로 KOBIS_API_KEY를 추가하거나 .env 파일을 구성해주세요." 
      });
    }
...
app.post("/api/review", async (req, res) => {
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({
        error: "Gemini API Key가 설정되지 않았습니다. Vercel 환경 변수로 GEMINI_API_KEY를 추가하거나 .env 파일을 구성해주세요."
      });
    }
